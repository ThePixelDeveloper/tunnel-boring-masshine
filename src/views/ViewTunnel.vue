    ...
    <div class="flex flex-col overflow-y-auto h-screen">
      ...
      <div class="p-3 pl-5 pr-5 border-b border-dashed">
        <div class="flex flex-row">
          ...
          <div class="flex flex-row text-sm items-center">
            <span
              v-bind:class="{
                'text-green-500': tunnel.status === 'Connected',
                'text-orange-500':
                  tunnel.status === 'Connecting' ||
                  tunnel.status === 'Disconnecting',
                'text-red-600': tunnel.status === 'Disconnected',
              }"
            >
              <svg
                class="fill-current"
      ...
      <div class="p-3 pl-5 pr-5 border-b border-dashed">
        <div class="flex flex-row">
          ...
          <div class="text-sm">
            {{ tunnel.username }}<span class="ml-1 mr-1 text-gray-600">@</span
            >{{ tunnel.hostname }}<span class="text-gray-600 ml-1 mr-1">:</span
            >{{ tunnel.port }}
          </div>
        </div>
      ...
      <div class="p-3 pl-5 pr-5 border-b border-dashed">
        <div class="flex flex-row">
          ...
          <div>
            ...
            <div
              class="flex flex-row items-center mt-4 mb-1"
              v-for="(rule, i) in tunnel.rules"
              :key="i"
            >
              <div
              ...
              >
                <span
                  class="pl-1 flex-auto text-sm text-right text-green-700 placeholder-green-700 bg-transparent"
                  >{{ rule.local.address }}</span
                >
                <span class="text-sm mx-1 text-green-700">:</span>
                <span
                  class="pr-1 text-sm text-green-700 placeholder-green-700 bg-transparent"
                  >{{ rule.local.port }}</span
                >
              </div>
              ...
              >
                <span
                  class="pl-1 flex-auto text-sm text-right text-blue-700 placeholder-blue-700 bg-transparent"
                  >{{ rule.target.address }}</span
                >
                <span class="text-sm mx-1 text-blue-700">:</span>
                <span
                  class="pr-1 text-sm text-blue-700 placeholder-blue-700 bg-transparent"
                  >{{ rule.target.port }}</span
                >
              </div>
            </div>
          </div>
      ...
      <div class="bg-gray-200 px-5 text-white h-10 flex row">
        ...
        <div class="flex flex-row-reverse items-center flex-grow">
          <router-link
            tag="button"
            :to="{ name: 'Edit Tunnel', params: { id: this.$route.params.id } }"
            class="font-medium text-xs shadow rounded py-1 px-5 bg-gray-100 text-blue-500"
            >Update
          </router-link>
          <button
            @click="remove"
            class="mx-1 font-medium text-xs shadow rounded py-1 px-5 bg-red-500 text-white"
          >
            Delete
          </button>
        </div>
