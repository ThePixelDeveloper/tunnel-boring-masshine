<template>
            ...
            <span v-bind:class="{'text-green-500': tunnel.status==='Connected', 'text-orange-500': tunnel.status==='Connecting', 'text-red-600': tunnel.status==='Disconnected'}">
              <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path
                      fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8z"></path></svg>
            </span>
                    </div>
                    <div class="flex-1 flex-col">
                        <h2 class="text-sm text-right text-gray-800 font-medium">{{tunnel.name}}</h2>
                        <p class="text-right text-sm text-gray-600"><span class="">{{tunnel.status}}</span></p>
                    </div>
                </div>

                <div class="flex-grow"></div>

                <div class="px-5 text-white h-10 flex flex-col">
                    <div class="flex flex-row-reverse items-center flex-grow text-gray-700">
                        <router-link to="/new-tunnel">
